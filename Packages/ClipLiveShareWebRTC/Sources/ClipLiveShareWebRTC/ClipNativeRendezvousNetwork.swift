...
public struct URLSessionClipLiveShareHTTPTransport: ClipLiveShareHTTPTransport {
    ...
    public func execute(_ request: URLRequest) async throws -> ClipLiveShareHTTPResult {
        let maximumBytes = ClipNativeRendezvousLimits.maximumHTTPResponseBytes
        ...
        if response.expectedContentLength > 0 {
            data.reserveCapacity(
                min(maximumBytes, Int(response.expectedContentLength))
            )
        }
    ...
    ClipLiveShareWebSocketFactory
{
    ...
    public func makeConnection(
        ...
        task.maximumMessageSize = ClipNativeRendezvousLimits.maximumMessageBytes
