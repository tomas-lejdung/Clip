...
public actor ClipNativeRendezvousOwnerTransport {
    ...
    private func performPublish(
        ...
        while true {
            do {
                ...
                break
            } catch ClipNativeRendezvousError.ownerOffline {
    ...
    private func performStopSharing(expectedGeneration: UInt64) async throws {
        ...
        if connection != nil {
            emit(.ownerPreparing(reconnectAttempt: 0))
        }
    }
    ...
    private func openConnection(
        ...
        emit(.connecting(role: .owner, reconnectAttempt: reconnectAttempt))
        var request = URLRequest(
            url: try capabilities.ownerWebSocketURL(for: owner.target)
        )
        ...
        resetPerConnectionState()
        emit(.connected(role: .owner, reconnectAttempt: reconnectAttempt))
        emit(.ownerPreparing(reconnectAttempt: reconnectAttempt))
    ...
    private func handle(
        ...
        case .ownerUnavailable, .closeRoute:
            rejectInvalidMessage()
        }
    }
