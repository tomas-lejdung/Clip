...
public actor ClipNativeRendezvousCandidateTransport {
    ...
    private func openConnection(
        ...
        emit(.connecting(role: .candidate, reconnectAttempt: reconnectAttempt))
        var request = URLRequest(
            url: try capabilities.candidateWebSocketURL(for: target)
        )
        ...
        inboundSequence = 0
        resetPerConnectionState()
        emit(.connected(role: .candidate, reconnectAttempt: reconnectAttempt))
